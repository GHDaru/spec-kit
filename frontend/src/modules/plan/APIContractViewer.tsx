...
function EndpointRow({ ep }: { ep: APIContract['endpoints'][number] }) {
  ...
  return (
    ...
    >
      ...
      {open && (ep.request_body || ep.response) && (
        <div
          style={{
            padding: '8px 12px',
            ...
            background: '#f9fafb',
            display: 'grid',
            gridTemplateColumns: ep.request_body && ep.response ? '1fr 1fr' : '1fr',
            gap: 12,
          }}
        >
          {ep.request_body && (
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
        ...
        <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>
          {contract.title} v{contract.version} · base: <code>{contract.base_path}</code>
        </span>
      </div>
